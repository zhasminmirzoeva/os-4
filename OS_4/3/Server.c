...
int ObserversCompare(const void *, const void *);
...
int ObserversCompare(const void *a, const void *b)
{
    ...
    do
    {
        ...
        if (RecvFromUnblocked(sock, msgBuffer, &recvMsgSize, &clientAddr))
        {
            ...
            if (strcmp(msgBuffer, "I_AM_OBSERVER") == 0)
            {
                ...
                if (!ListContains(library.observers, &obs, ObserversCompare))
                {
                    // Add a new observer
            ...
            {
                // The observer wants to disconnect
                Observer obs;
                obs.addr = clientAddr;
                ...
                if (ListContains(library.observers, &obs, ObserversCompare))
                {
                    ...
                    Observer *obsItem = ListRemove(library.observers, &obs, ObserversCompare);
                    free(obsItem);                    
                }
