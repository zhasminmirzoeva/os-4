...
void NotifyObserver(Library *library, const char *msg);
...
void NotifyObserver(Library *library, const char *msg)
{
    if (library->observer.connected)
    {
        SendTo(library->sock, msg, strlen(msg), &library->observer.addr);
    }    
    printf("%s\n", msg);
}
...
void PrintCatalog(Library *library)
{
    ...
    NotifyObserver(library, "The recovered catalog is:");
    ...
    while (node)
    {
        ...
        NotifyObserver(library, notifyBuffer);
        node = node->next;
    }
}
    ...
    do
    {
        ...
        if (RecvFromUnblocked(sock, msgBuffer, &recvMsgSize, &clientAddr))
        {
            ...
            if (strcmp(msgBuffer, "I_AM_OBSERVER") == 0)
            {
                ...
                library.observer.connected = 1;
                library.observer.addr = clientAddr;
